...
def print_outcome(associated_rules, attribute_list, output_file):
  f = open(output_file, 'w')
  f.write("Association Rule\tConfidence\n")
...
def cal_brute_force_num(associated_rules):
  total = 0
  for conf, rules in associated_rules.items():
    for i in range(len(rules)):
      k = 0
      k = len(rules[i][0]) + len(rules[i][1])
      tmp = 2**k - 2
      total = total + tmp
  print "By brute-force method, it will generate %d candidates" %total
  
#Main function starts from here
if len(sys.argv) != 11:
		print "USAGE: q3_c.py -i [INPUT_FILE] -m [MIN_SUPPORT] -c [MIN_CONFIDENCE] -o [OUTPUT_FILE] -f [METHOD]"
		exit(-1)
...
if input_file == "item_1000.txt":
  ...
  print_outcome(associated_rules, name_list, output_file)
  cal_brute_force_num(associated_rules)
  ...
  associated_rules = generate_rules(freq_item, min_conf, method)
  print_outcome(associated_rules, attribute_list, output_file)
  cal_brute_force_num(associated_rules)
  print "The result is in the output file"
...
#detail = read_input("item_100.txt", "goods.txt")
#min_sup = 5
#print detail
#name_list = read_name("goods.txt")
#print name_list
#freq_item = apriori(detail, min_sup) 
#associated_rules = generate_rules(freq_item, 0.6)
